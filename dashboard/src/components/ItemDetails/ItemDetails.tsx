...
export function ItemDetails({
  ...
  const { status: prStatus, loading: prLoading } = usePrStatus(item.pr_url)
  const { stack: prStack, loading: stackLoading } = usePrStack(item.pr_url, isStack)
  ...
  const [prStatusFetched, setPrStatusFetched] = useState<{ state?: string; reviewDecision?: string; checks?: string } | null>(null)
  ...
  useEffect(() => { if (!editingNotes) setNotesText(notes || '') }, [notes, editingNotes])
  ...
  useEffect(() => { if (editingDescription && descRef.current) { descRef.current.focus(); descRef.current.selectionStart = descRef.current.value.length } }, [editingDescription])
  useEffect(() => { if (editingNotes && notesRef.current) { notesRef.current.focus(); notesRef.current.selectionStart = notesRef.current.value.length } }, [editingNotes])

  // Fetch PR status for panel variant (DetailPanel's original fetch)
  useEffect(() => {
    if (variant !== 'panel' || !item.pr_url) return
    const url = encodeURIComponent(item.pr_url)
    fetch(`/api/pr-status?url=${url}`)
      .then(res => res.ok ? res.json() : null)
      .then(data => { if (data) setPrStatusFetched(data) })
      .catch(() => {})
  }, [item.pr_url, variant])
  ...
  return (
    <div className={classnames(styles.Root, styles[variant])} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.Section}>
          <div className={styles.SectionHeader}>
            ...
            {onUpdateBlockedBy && !showDepForm && (
              <button className={styles.EditButton} onClick={() => setShowDepForm(true)}>
            ...
            {item.blocked_by.map(depId => {
              ...
              return (
                <div key={depId} className={styles.DepItem}>
                  <span className={classnames(styles.DepDot, isResolved && styles.DepResolved)} />
                  ...
                  {onUpdateBlockedBy && (
                    <button
                      ...
                      onClick={() => onUpdateBlockedBy(item.id, item.blocked_by.filter(id => id !== depId))}
      ...
      {showDepForm && onUpdateBlockedBy && (
        <div className={styles.InlineForm}>
          ...
          <div className={styles.FormActions}>
            ...
            <button className={styles.CancelButton} onClick={() => { setDepInput(''); setShowDepForm(false) }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {/* Implementation Plan */}
      {(itemPlanFile || onGeneratePlan || item.status === 'planning' || item.status === 'queued') && (
        <div className={styles.Section}>
          <div className={styles.SectionHeader}>
            <h4 className={styles.SectionTitle}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className={styles.PlanActions}>
              {onGeneratePlan && !itemPlanFile && (
                <button className={styles.EditButton} onClick={() => onGeneratePlan(item.id)}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M12 2L2 7l10 5 10-5-10-5z" />
                    <path d="M2 17l10 5 10-5" />
                    <path d="M2 12l10 5 10-5" />
              ...
              {itemPlanFile && (
                <button className={styles.EditButton} onClick={handleOpenPlanFile} title="Open plan file in editor">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {!itemPlanFile && <div className={styles.PlanEmpty}>No plan file yet. Generate one or create manually.</div>}
        </div>
      )}
        ...
        <div className={styles.Section}>
          <div className={styles.SectionHeader}>
            ...
            {onEdit && !editingDescription && (
              <button className={styles.EditButton} onClick={() => setEditingDescription(true)}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className={styles.InlineForm}>
              ...
              <div className={styles.FormActions}>
                <button
                  ...
                  onClick={() => {
                    if (onEdit) onEdit(item.id, { description: descriptionText })
            ...
            <>
              {item.description ? (
                ...
                <span className={styles.EmptyText}>
                  {onEdit ? 'Click edit to add a description' : 'No description'}
                </span>
              )}
            </>
          )}
      ...
      <div className={styles.Section}>
        <div className={styles.SectionHeader}>
          ...
          {onNotesChange && !editingNotes && (
            <button className={styles.EditButton} onClick={() => setEditingNotes(true)}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
        ...
        {editingNotes ? (
          <div className={styles.InlineForm}>
            ...
            <div className={styles.FormActions}>
              ...
              <button className={styles.CancelButton} onClick={() => { setEditingNotes(false); setNotesText(notes || '') }}>Cancel</button>
            </div>
          </div>
        ) : (
          <>
      ...
      {linkedSession && (
        <div className={styles.Section}>
          ...
          <div className={styles.SessionCard}>
            <span className={classnames(styles.SessionDot, styles[`session_${linkedSession.state}`])} />
      ...
      <div className={styles.ActionBar}>
        ...
        <div className={styles.StatusActions}>
          ...
          {item.status === 'queued' && onActivateStream && (
            <button
              className={classnames(styles.ActionButtonText, styles.ActionPrimary)}
